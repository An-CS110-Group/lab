...
v_add_optimized_chunks (double *x, double *y, double *z)
{
  ...
  {
    int num_threads = omp_get_num_threads();
    int tasksize = ARRAY_SIZE/num_threads;
    int id = omp_get_thread_num();
    
    int start = id*tasksize;
    int end = (id!=num_threads-1)?(id+1)*tasksize:ARRAY_SIZE;
    for (int i = start; i < end; i++)
      z[i] = x[i] + y[i];
  }
}

// Using openMP built-in method for testing
void
v_add_optimized_chunks_auto (double *x, double *y, double *z)
{
